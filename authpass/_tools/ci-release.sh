...
#
# Runs *inside* `cux_ship keychain exec` — see .github/workflows/ios.yaml.
# Since cux_ship 3.0.0 that passes the keychain and nothing else: the only
# credential in here is the deploy key, named at the call site with
# `--only ssh_keys.github_deploy` because the build number push needs it.
# Nothing here decrypts anything — the sops identity is stripped before this
# runs — and nothing here holds a secret that outlives the run.
...
# The deploy key arrives as a path, materialized by `cux_ship keychain exec
# --only ssh_keys.github_deploy` into a temp directory it removes however this
# exits — so there is nothing to chmod and nothing left behind afterwards.
